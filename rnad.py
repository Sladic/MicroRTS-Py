...
class Agent(nn.Module):
    def __init__(self, envs, mapsize=16 * 16, partial_obs=False):
        super(Agent, self).__init__()
        self.mapsize = mapsize
        h, w, c = envs.observation_space.shape
        if partial_obs:
            c = 32  # Set the correct number of channels for partial obs
        ...
        self.rnn = nn.LSTM(64 * (h // 4) * (w // 4), 128, batch_first=True)
    ...
    def get_action_and_value(self, x, hxs, masks, action=None, invalid_action_masks=None, envs=None, device=None):
        hidden = self.encoder(x)
        hidden = hidden.view(hidden.size(0), -1)
        rnn_out, hxs = self.rnn(hidden.unsqueeze(0), (hxs * masks).unsqueeze(0))
        ...
        return action, logprob.sum(1).sum(1), entropy.sum(1).sum(1), invalid_action_masks, self.critic(rnn_out), hxs

    def get_value(self, x, hxs, masks):
        hidden = self.encoder(x)
        hidden = hidden.view(hidden.size(0), -1)
        rnn_out, hxs = self.rnn(hidden.unsqueeze(0), (hxs * masks).unsqueeze(0))
        rnn_out = rnn_out.squeeze(0)
        return self.critic(rnn_out), hxs
...
if __name__ == "__main__":
    ...
    rnn_hidden_states = torch.zeros((args.num_steps, args.num_envs, 128)).to(device)
    next_obs = torch.Tensor(envs.reset()).to(device)
    next_done = torch.zeros(args.num_envs).to(device)
    next_rnn_hidden_states = torch.zeros((args.num_envs, 128)).to(device)
    ...
    for update in range(starting_update, args.num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            rnn_hidden_states[step] = next_rnn_hidden_states

            with torch.no_grad():
                invalid_action_masks[step] = torch.tensor(envs.get_action_mask()).to(device)
                action, logproba, _, _, vs, next_rnn_hidden_states = agent.get_action_and_value(
                    next_obs, next_rnn_hidden_states, next_done, envs=envs, invalid_action_masks=invalid_action_masks[step], device=device
                )
        ...
        with torch.no_grad():
            last_value, _ = agent.get_value(next_obs, next_rnn_hidden_states, next_done)
        ...
        b_rnn_hidden_states = rnn_hidden_states.reshape((-1, 128))
        ...
        for i_epoch_pi in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogproba, entropy, _, new_values, _ = agent.get_action_and_value(
                    b_obs[minibatch_ind], b_rnn_hidden_states[minibatch_ind], torch.ones(len(minibatch_ind)).to(device),
                    b_actions.long()[minibatch_ind], b_invalid_action_masks[minibatch_ind], envs, device
                )
